...
export default async function AcademyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="skillcity-shell-bg text-brand-dark"><Navbar /><section className="px-5 py-16 sm:px-8 lg:px-10"><Container><div className="grid gap-10 lg:grid-cols-[1fr_340px]"><div><p className="text-sm font-black uppercase text-brand-red">Academy</p><h1 className="mt-4 text-5xl font-black leading-tight md:text-7xl">{academy.title}</h1><p className="mt-6 max-w-3xl text-xl leading-9 text-brand-muted">{academy.mission}</p>{academy.comingSoon ? <p className="mt-6 inline-flex rounded-lg bg-brand-beige px-4 py-2 font-black text-brand-red">Coming Soon</p> : null}</div><Card><CardContent className="grid min-h-60 place-items-center p-8"><Icon className="h-20 w-20 text-brand-red" /></CardContent></Card></div></Container></section><Container className="space-y-12 pb-16"><Block title="Mission" items={[academy.mission]} /><Block title="Programs" items={academy.programs} links={linkedPrograms} /><Block title="Career Paths" items={academy.paths} /><Block title="Future Roadmap" items={academy.roadmap} /><div className="flex flex-col gap-4 sm:flex-row"><Button asChild size="lg"><Link href={getAcademyApplyHref(academy.slug)}>{academy.comingSoon ? "Join Waitlist" : getAcademyApplyLabel(academy.slug)}</Link></Button><Button asChild size="lg" variant="secondary"><Link href="/#academies">View All Academies</Link></Button></div></Container><Footer /></main>;
}
...
function getAcademyApplyHref(slug: string) {
  if (slug === "aira-labs") return "/apply?program=aira-labs";
  return "/apply?program=startup-skool";
}

function getAcademyApplyLabel(slug: string) {
  if (slug === "aira-labs") return "Apply for AIRA Labs";
  return "Apply for Startup Skool";
}
