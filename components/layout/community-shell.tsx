...
export function CommunityShell({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-screen bg-white text-brand-dark">
      ...
      <header className="sticky top-0 z-30 border-b border-black/5 bg-white/95 px-5 py-4 xl:hidden"><Link href="/community-hub/feed"><Logo /></Link><nav className="mt-4 flex gap-2 overflow-x-auto pb-1">{nav.map((item) => <Link key={item.href} href={item.href} className="shrink-0 rounded-lg border border-black/10 px-4 py-2 text-sm font-bold text-brand-muted">{item.label}</Link>)}</nav></header>
