...
import { assessmentReviewSchema, attendanceRecordSchema, attendanceSessionSchema, reflectionReviewSchema, resourceSchema, studentConcernSchema, submissionReviewSchema, trainerAnnouncementSchema } from "@/features/trainer/schemas";
...
export async function markAttendanceAction(previousState: State = state, formData: FormData): Promise<State> {
  ...
  await ensureStudentAccess(trainer.id, parsed.data.studentId);
